...
def complete_job(job_id):
    ...
    job_progress[job_id] = (job_progress[job_id][0],
                            job_progress[job_id][1], True)
        ...
        if 'annotation_file' in request.files:
            ...
            if annotation_file.filename != '':
                annotation_file_path = os.path.join(
                    temp_dir, annotation_file.filename)
...
def generate_report_list(df, job_id, pdf_file_zip, annotation_file):
    ...
    try:
        for index, row in df.iterrows():
            ...
            try:
                ...
                for column_name in df.columns:
                    if column_name != 'id' and column_name != 'report' and column_name != 'metadata' and column_name != 'report_redacted' and column_name != 'masked_report':
                        personal_info_dict[column_name] = df[df['id']
                                                             == row['id']][column_name].item()
            ...
            personal_info_dict = {key: convert_personal_info_list(
                value) for key, value in personal_info_dict.items()}

            orig_pdf_path = os.path.join(pdf_file_zip, f"{row['id']}.pdf")

            print("Load Annotated PDF")
            report_dict['annotated_pdf_filepath'], report_dict['annotated_text_labelwise'], report_dict['original_text'], report_dict['colormap'], sofastring = load_annotated_pdf(
                row['id'], orig_pdf_path, annotation_file)
            ...
            for key, value in report_dict['dollartext_redacted_dict'].items():
                if key in report_dict['annotated_text_labelwise'].keys():
                    report_dict['scores'][key] = generate_score_dict(
                        report_dict['annotated_text_labelwise'][key], value, report_dict['original_text'])
                else:
                    print("Key not found in annotated_text_labelwise: ",
                          key, " in report: ", row['id'])
        ...
        for label, value in report_summary_dict['accumulated_metrics'].items():
            ...
            generate_confusion_matrix_from_counts(report_summary_dict['accumulated_metrics'][label]['true_positives'], report_summary_dict['accumulated_metrics'][label]['true_negatives'],
                                                  report_summary_dict['accumulated_metrics'][label]['false_positives'], report_summary_dict['accumulated_metrics'][label]['false_negatives'], confusion_matrix_filepath)
...
def accumulate_metrics(report_list):
    ...
    metrics = ['true_positives', 'false_positives', 'true_negatives', 'false_negatives', 'precision',
    ...
    for label, score_dict in labelwise_metrics.items():
        ...
        labelwise_metrics[label]['micro_precision'] = score_dict['true_positives'] / (
            score_dict['true_positives'] + score_dict['false_positives']) if (score_dict['true_positives'] + score_dict['false_positives']) != 0 else 0
        labelwise_metrics[label]['micro_recall'] = score_dict['true_positives'] / (
            score_dict['true_positives'] + score_dict['false_negatives']) if (score_dict['true_positives'] + score_dict['false_negatives']) != 0 else 0
        labelwise_metrics[label]['micro_accuracy'] = (score_dict['true_positives'] + score_dict['true_negatives']) / (score_dict['true_positives'] + score_dict['true_negatives'] + score_dict['false_positives'] +
                                                                                                                      score_dict['false_negatives']) if (score_dict['true_positives'] + score_dict['true_negatives'] + score_dict['false_positives'] + score_dict['false_negatives']) != 0 else 0
        labelwise_metrics[label]['micro_f1_score'] = 2 * labelwise_metrics[label]['micro_precision'] * labelwise_metrics[label]['micro_recall'] / (
            labelwise_metrics[label]['micro_precision'] + labelwise_metrics[label]['micro_recall']) if (labelwise_metrics[label]['micro_precision'] + labelwise_metrics[label]['micro_recall']) != 0 else 0
        labelwise_metrics[label]['micro_specificity'] = score_dict['true_negatives'] / (
            score_dict['true_negatives'] + score_dict['false_positives']) if (score_dict['true_negatives'] + score_dict['false_positives']) != 0 else 0
        labelwise_metrics[label]['micro_false_positive_rate'] = score_dict['false_positives'] / (
            score_dict['false_positives'] + score_dict['true_negatives']) if (score_dict['false_positives'] + score_dict['true_negatives']) != 0 else 0
        labelwise_metrics[label]['micro_false_negative_rate'] = score_dict['false_negatives'] / (
            score_dict['false_negatives'] + score_dict['true_positives']) if (score_dict['false_negatives'] + score_dict['true_positives']) != 0 else 0
...
def report_redaction_viewer(report_id):
    ...
    next_id = df.at[current_index + 1,
                    'id'] if current_index < len(df) - 1 else None

    orig_pdf_path = os.path.join(pdf_file_zip, f"{report_id}.pdf")
    if session.get('annotation_file', None) is None:
        session['redacted_pdf_filename'], dollartext_redacted_dict, personal_info_dict, fuzzy_matches_dict = load_redacted_pdf(personal_info_dict, orig_pdf_path, df, report_id, enable_fuzzy=session.get(
            'enable_fuzzy', False), threshold=session.get('threshold', 90), exclude_single_chars=session.get('exclude_single_chars', False), scorer=session.get('scorer', None))

    if session.get('annotation_file', None):
        try:
            session['annotation_pdf_filepath'], dollartext_annotated_labelwise, original_text, colormap, sofastring = load_annotated_pdf(
                report_id, pdf_file_zip, session['annotation_file'])
            session['redacted_pdf_filename'], dollartext_redacted_dict, personal_info_dict, fuzzy_matches_dict = load_redacted_pdf(personal_info_dict, orig_pdf_path, df, report_id, enable_fuzzy=session.get(
                'enable_fuzzy', False), threshold=session.get('threshold', 90), exclude_single_chars=session.get('exclude_single_chars', False), scorer=session.get('scorer', None), text=sofastring)
...
def load_annotated_pdf(report_id, pdf_file, annotation_zip_file):
    ...
    annotation_pdf_filepath, dollartext_annotated, original_text, dollartext_annotated_labelwise = annoparser.apply_annotations_to_pdf(
        pdf_file)
...
def load_redacted_pdf(personal_info_dict, filename, df, id, exclude_single_chars=False, enable_fuzzy=False, threshold=90, scorer="WRatio", text=None):
    ...
    if enable_fuzzy:
        for key, value in personal_info_dict.items():
            fuzzy_matches_dict[key] = find_fuzzy_matches(
                df.loc[df['id'] == id, 'report'].iloc[0], value, threshold=int(threshold), scorer=scorer)
    ...
    for key, value in personal_info_dict.items():
        dollartext_redacted_dict[key] = generated_dollartext_stringlist(
            filename, value, fuzzy_matches_dict[key], ignore_short_sequences=1 if exclude_single_chars else 0, text=text)
    ...
    anonymize_pdf(filename, personal_info_dict['personal_info_list'], filename.replace(
        ".pdf", "_redacted.pdf"), fuzzy_matches_dict['personal_info_list'])
...
def reportredactionconfusionmatrix():
    ...
    if job_id:
        ...
        confusion_matrix_svg_filepath = session.get(
            'confusion_matrix_filepath', None)
