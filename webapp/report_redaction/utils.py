...
class InceptionAnnotationParser:
        ...
        for custom_span in self.cas.select('custom.Span'):
            ...
            for pdf_chunk in self.cas.select('org.dkpro.core.api.pdf.type.PdfChunk'):
                ...
                if span_begin <= chunk_end and span_end >= chunk_begin:
                    ...
                    print(f"Custom Span '{custom_span.label}' Bounding Box: x={x_start}, y={
                          pdf_chunk.y}, width={x_end-x_start}, height={pdf_chunk.h}, page={page_number}")
                    bounding_box = (
    ...
    def convert_annotations(self, text_with_extra, clean_text, annotations):
        ...
        for annotation in annotations:
            ...
            clean_begin = begin - \
                self.compare_text_and_count_removals(diff, begin)
            ...
            try:
                ...
                print(
                    "Error: Length of converted text positions does not look right! ", e)
                breakpoint()

            converted_annotations.append({'begin': clean_begin, 'end': clean_end,
                                         'label': annotation['label'], 'coveredText': annotation['coveredText']})
    ...
    def merge_bounding_boxes_within_range(self, doc, start_pos, end_pos, annotation_text):
        print("Run for annotation text: " + annotation_text +
              " with start pos: " + str(start_pos) + " and end pos: " + str(end_pos))
        merged_bboxes = []
                ...
                for l, line in enumerate(word_block['lines']):
                    for span in line['spans']:
                        ...
                        if word_line_end_pos > start_pos and word_line_start_pos <= end_pos:
                            print("Found match for annotation text: " + annotation_text +
                                  " in line " + str(l) + " of page " + str(page_num))
                            # Calculate the adjusted start and end positions for this word
                            adjusted_start_pos = max(start_pos - char_count, 0)
                            # TODO: removed word_line - 1
                            adjusted_end_pos = min(
                                end_pos - char_count, len(word_line))
                            ...
                            if len(cut_from_text) == 0:
                                ...
                                print("Found non-empty text match: '" +
                                      cut_from_text + "'")
                            ...
                            if len(bboxes) > 0:
                                ...
                                search_mask_expanded = fitz.Rect(
                                    expand_search_mask(search_mask))

                                if cut_from_text in annotation_text:
                                    annotation_in_text_match = page.search_for(
                                        cut_from_text, clip=search_mask_expanded)
                                else:
                                    breakpoint()
                                    raise Exception(
                                        "Annotation text does not match the text gathered from the PDF.")

                                if not annotation_in_text_match:
                                    print(
                                        "Did not find text in the bounding box: ", cut_from_text)
                                    print(
                                        "Iteratively enlargeing the bounding box.")
                                    for i in range(10):
                                        search_mask_expanded = fitz.Rect(
                                            expand_search_mask(search_mask_expanded, i*3, i))
                                        annotation_in_text_match = page.search_for(
                                            cut_from_text, clip=search_mask_expanded)
                                    ...
                                    if not annotation_in_text_match:
                                        print(
                                            "Did not find text in the bounding box: ", cut_from_text)
                                        raise Exception(
                                            "Did not find text in the bounding box, even after enlarging it. Text: ", cut_from_text)

                                elif len(annotation_in_text_match) == 1:
                                    merged_bboxes.append(
                                        (page_num, annotation_in_text_match[0]))
                                elif len(annotation_in_text_match) > 1:
                                    print(
                                        "More than one match in bounding box. Ambiguous. Choose first.")
                                    merged_bboxes.append(
                                        (page_num, annotation_in_text_match[0]))
                                    # breakpoint()
                                    ...
                                    breakpoint()
                                    print(
                                        "No match, use inaccurrate bounding boxes.")
                                    merged_bboxes.append(
                                        (page_num, search_mask))

                        # Update the character count
                        # TODO: +1 only if additional spaces per line are used!
                        char_count += len(word_line) + 1
    ...
    def apply_annotations_to_pdf_old(self, pdf_input_path):
        ...
        for annotation in annotations:
            annotation['coveredText'] = sofastring[annotation['begin']
                :annotation['end']]
        ...
        for anno in anex:
            # try:
            assert anno['coveredText'].replace("\r\n", " ").replace("\n", " ") == anno['extracted_text'], f"The original text '{
                anno['coveredText']}' does not match the extracted text with the updated positions '{anno['extracted_text']}'"
            # except AssertionError as e:
            #     breakpoint()
        ...
        for annotation in tqdm(anconv):
            bboxes = self.merge_bounding_boxes_within_range(
                pdf, annotation['begin'], annotation['end'], annotation_text=annotation['extracted_text'])
        ...
        self.overlay_annotations(
            pdf_input_path, overlay_output_file, anconv, colormap)
...
def calculate_metrics(ground_truth, automatic_redacted, original_text, redacted_char):
    ...
    accuracy = (true_positives + true_negatives) / (true_positives + true_negatives +
                                                    false_positives + false_negatives)  # Ignoring spaces in original text
